...
def train():
    ...
    if CHECKPOINT_LOAD and os.path.exists(PPO_MODEL_PATH + '.zip'):
        print("Loading model from {} ...".format(PPO_MODEL_PATH))
        model = PPO.load(
            PPO_MODEL_PATH,
            env=env,
    ...
    # ── Train ─────────────────────────────────────────────────────────
    print("\nTraining for {:,} timesteps ...\n".format(int(TRAIN_TIMESTEPS)))
    model.learn(
        total_timesteps     = int(TRAIN_TIMESTEPS),
        callback            = callbacks,
        reset_num_timesteps = not CHECKPOINT_LOAD,
        tb_log_name         = "BLIP_FusePPO",
        # progress_bar removed — requires 'rich' package
        # install with: pip install rich   to re-enable
    )

    # ── Save ──────────────────────────────────────────────────────────
    model.save(PPO_MODEL_PATH)
    print("\nModel saved to {}".format(PPO_MODEL_PATH))
    env.close()
...
def test():
    ...
    while episode < TEST_EPISODES:
        ...
        print("Ep {:3d} | Reward: {:7.2f} | Dist: {:5.0f}m | Dev: {:.4f}m".format(
            episode, ep_reward, dist, dev))

    print("\n── Test Summary ──────────────────────────────")
    print("Mean Reward:    {:.2f}".format(np.mean(total_rewards)))
    print("Mean Distance:  {:.0f}m".format(np.mean(total_distances)))
    print("Mean Deviation: {:.4f}m".format(np.mean(total_deviations)))
    print("RMSE Deviation: {:.4f}m".format(
        np.sqrt(np.mean(np.array(total_deviations)**2))))
